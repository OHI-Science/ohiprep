...
r_MPAc13<- r_MPA %>% filter(year<2013) %>% group_by(rgn_id,tot_area_km2) %>%  summarise(mpa_area=sum(area_km2)) %>% mutate(scen='v2013')
r_MPAc14<-r_MPA %>% group_by(rgn_id,tot_area_km2) %>%  summarise(mpa_area=sum(area_km2)) %>% mutate(scen='v2014')
