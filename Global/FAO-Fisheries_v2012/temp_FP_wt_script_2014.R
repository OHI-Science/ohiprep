...
# calculate total MAR catch per OHI region in 2010 (2012a), 2011 (2013a, 2014a)
...
newSAUP2 <- rename(newSAUP, c("yr"="year","EEZ"="SAUP_C_NUM")) # this change to the 'newSAUP' file was performed in the CMSY data prep script

newSAUP2<-filter(newSAUP2,SAUP_C_NUM>0)
saup_to_ohi <- read.csv(file.path(dir_FIS_data, 'EEZlookup.csv')) ; head(saup_to_ohi)
newSAUP2<-left_join(newSAUP2,select(saup_to_ohi,SAUP_C_NUM,OHI_2013_EEZ_ID)) # add species names # Joining by: "Taxonkey"
...
FPdata3<-join(FISdata3,MARdata3) # Joining by: rgn_id_2013, year
...
fp_wildcaught_weight_2012a <- FPdata3 %>% filter(year==2010) %>% select(rgn_id,w_fis)
fp_wildcaught_weight_2013a <- FPdata3 %>% filter(year==2011)  %>% select(rgn_id,w_fis)
anyDuplicated(fp_wildcaught_weight_2012a[,1])
anyDuplicated(fp_wildcaught_weight_2013a[,1])

write.csv(fp_wildcaught_weight_2012a,file.path(dir_d, 'GL_FP_wildcaught_weight_v2012a_lyr.csv'),row.names=F)
write.csv(fp_wildcaught_weight_2013a, file.path(dir_d, 'GL_FP_wildcaught_weight_v2013a_lyr.csv'),row.names=F)
