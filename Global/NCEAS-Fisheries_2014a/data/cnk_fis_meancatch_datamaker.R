...
source('Global/NCEAS-Fisheries_2014a/tmp/CMSY data prep.R')
...
nS.eez$stock_id_eez<-paste(nS.eez$stock_id,nS.eez$EEZ,sep='_')
# sum catch (for 'Marine fishes not identified', 'Shrimps and prawns', 'Sharks rays and chimaeras' there are duplicate taxonkeys)
nS.eez<-nS.eez %>% group_by(year, stock_id_eez, EEZ, FAO, stock_id, Taxonkey, TaxonName, TLevel) %>% summarise (Catch2=sum(Catch))

stockss<-as.data.frame(cbind('stock_id_eez'=unique(nS.eez$stock_id_eez),'join'=rep(1,times=length(unique(nS.eez$stock_id_eez)))))
yr_stck<-join(yrs,stockss)# Joining by: "join"

nS2<-join(yr_stck[,c(1,3)],nS.eez) ; head(nS2) # Joining by: year, stock_id_eez
# dim(nS2[is.na(nS2$Catch2),]) # check how many NAs added : 482283
names(nS2)[names(nS2)=='Catch2']<-'Catch'

min_yr<-ddply(nS2, .(stock_id_eez),summarise,min_year=min(year[!is.na(Catch)]))
nS3<-join(nS2,min_yr) # Joining by: stock_id_eez
...
####### need to change this because the current data have unique values per taxonkey but not taxonname - will ahve to check the following code too, 
# because the output file 'cnk_fis_meancatch' no longer will have the same code 'taxon_name_key' (I can preserve the same variable name, but it'll just be the taxonkey)
#########################
...
country.level.data<-join(country.level.data, MeanCatch, by=c("saup_id","fao_id","Taxonkey"))
...
cnk_fis_meancatch <- subset(country.level.data, select=c("fao_saup_id", "taxon_key", 
                                                         "year", "mean_catch")) 
