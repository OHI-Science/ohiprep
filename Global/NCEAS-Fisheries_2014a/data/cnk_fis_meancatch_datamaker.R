...
nS.eez<-nS.eez %>% group_by(year, EEZ, FAO, stock_id, Taxonkey, TaxonName, TLevel) %>% summarise (Catch2=sum(Catch))
# dim(nS2[is.na(nS2$Catch2),]) # check how many NAs added : 482283
names(nS.eez)[names(nS.eez)=='Catch2']<-'Catch'
...
yr_stck<-join(yrs,stockss)# Joining by: "join" # beware: this uses plyr's 'join' instead of dplyr because I want rows for all multiple correspondences between years and ids

nS2<-join(yr_stck[,c(1,3)],country.level.data) ; head(nS2) # Joining by: year, taxon_name_key_id # same comment as above about plyr's join fn
...
nS3$Catch<-ifelse(is.na(nS3$Catch),ifelse(nS3$year<nS3$min_year,NA,0),nS3$Catch) ; dim(nS3) # pad with 0s
...
write.csv(cnk_fis_meancatch,file.path(dir_d,'data/cnk_fis_meancatch.csv'),row.names=F)
