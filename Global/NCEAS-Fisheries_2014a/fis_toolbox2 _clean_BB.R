...
# > u = read.csv('cnk_fis_meancatch.csv'        , na.strings=''); head(u); paste(names(u), collapse="','") # 'fao_saup_id','TaxonName_TaxonKey','year','MeanCatch'
# fao_saup_id                   TaxonName_TaxonKey year    MeanCatch
# 1      37_100                 Marine animals_1e+05 2006 2.427059e+00
# 2      37_100        Sharks, rays, chimaeras_6e+05 2006 5.042719e+01
# 3      37_100         Homaridae/Palinuridae_100043 2006 5.778884e-03
# 4      37_100     Clams, cockles, arkshells_100056 2006 5.857143e-01
# 5      37_100 Miscellaneous marine molluscs_100058 2006 2.722479e+02
# 6      37_100   Marine fishes not identified_6e+05 2006 3.128928e+02
# [1] "fao_saup_id','TaxonName_TaxonKey','year','MeanCatch"
# > b = read.csv('fnk_fis_b_bmsy.csv'           , na.strings=''); head(b); paste(names(b), collapse="','") # 'fao_id','TaxonName','year','b_bmsy'
# fao_id      TaxonName year   b_bmsy
# 1     51 Ablennes hians 1985 1.232531
# 2     51 Ablennes hians 1990 1.288008
# 3     51 Ablennes hians 1991 1.307404
# 4     51 Ablennes hians 1992 1.274851
# 5     51 Ablennes hians 1993 1.132222
# 6     51 Ablennes hians 1994 1.028887
# [1] "fao_id','TaxonName','year','b_bmsy"
# > s = read.csv('snk_fis_propArea_saup2rgn.csv', na.strings=''); head(s); paste(names(s), collapse="','") # 'fao_saup_id','TaxonName_TaxonKey','year','MeanCatch'
# saup_id rgn_id_2013 propArea
# 1     166           1        1
# 2     162           2        1
# 3     574           3        1
# 4      37           4        1
# 5     540           5        1
# 6     548           6        1
# [1] "saup_id','rgn_id_2013','propArea"


#setwd("C:\\Users\\Melanie\\Desktop\\Revising FIS")
setwd('/Volumes/data_edit/model/GL-NCEAS_FIS_v2013a/RevisingFIS')
...
#                       status.year=2011){
ld.csv = 'data/layers_data.csv'
status.year=2011

# layers
  lyrs = list('rk'=c(
    'snk_fis_meancatch'= "catch", 
    'fnk_fis_b_bmsy' = "bmsy", 
    'snk_fis_proparea_saup2rgn'="prop_area"))
  layers = sub('(r|ry|rk)\\.','', names(unlist(lyrs)))
  
#head(SelectLayersData(layers, layers='snk_fis_meancatch', narrow=T))
...
Status <- StatusData[StatusData$year==as.character(status.year), ]
