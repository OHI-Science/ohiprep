...
for (scenario in c('eez2012','eez2013','eez2014')){ # scenario  = 'eez2013'
  ...
  # swap tonnes for usd if still NA, for extreme cases (eg 2013 Democratic Republic of the Congo [199] with only 1 or 0 harvest tonnes but legit usd producing near perfect scores)
  h = h %>%
    mutate(
      tonnes = ifelse(is.na(tonnes), usd, tonnes))
