...
q1 = rad %.%
  ...
  dcast(region_name + region_id ~ scenario, value.var='value') %.%
...
# query
q2 = anti_join(status_metric_2013, status_metric_2012, by=c('metric','iso3166')) %.%
  filter(metric=='rev') %.%
  arrange(score)
...
# write
write.csv(q3, file.path(dir_out, 'q1.csv'))
...
# query
q5 = anti_join(status_metric_2013, status_metric_2012, by=c('metric','iso3166')) %.%
  filter(metric=='jobs')
...
# query
q6 = anti_join(status_metric_2013, status_metric_2012, by=c('metric','iso3166')) %.%
  filter(metric=='wage') %.%
  arrange(score)
