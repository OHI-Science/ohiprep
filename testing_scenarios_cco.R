...
status_line <- ggplot(scores_df %>% filter(dimension == dim_test), 
...
if(nrow(incr_df) > 0) {
  incr_status_line <- ggplot(incr_df, 
    ...
    # scale_color_brewer(palette = 'RdYlBu') +
    labs(color = 'georegion',
         title = paste0('increase in ', goalname, ' ', dim_test, '; n = ', nrow(same_df)))
...
if(nrow(decr_df) > 0) {
  decr_status_line <- ggplot(decr_df, 
    ...
    # scale_color_brewer(palette = 'RdYlBu') +
    labs(color = 'georegion',
         title = paste0('decrease in ', ' ', dim_test, '; n = ', nrow(same_df)))
...
if(nrow(same_df) > 0) {
  ...
  same_status_pt <- ggplot(same_df %>% 
    ...
    # scale_color_brewer(palette = 'RdYlBu') +
    labs(color = 'georegion',
         title = paste0('no change in ', ' ', dim_test, '; n = ', nrow(same_df)))
