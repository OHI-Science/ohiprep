for (p in poles){ 
  ...
  ###########################################################
  ## select August rasters
  ###########################################################
  AugMonths <- grep("08$", names(s), value=TRUE)
  si <- subset(s, AugMonths)
  ...
  write.csv(z.h.T, sprintf('tmp\\%s_IceEdgeHabitat_august.csv',p), row.names=FALSE)
