...
dir_AQ <- '../ohiprep/Antarctica/AQ_FIS_v2013'
...
CCAMLR_FIS<-read.csv(file.path(dir_AQ, "raw/CCAMLR_w_update_v2.csv") )
length(unique(CCAMLR_FIS$SpeciesCode)) # count unique taxa: 225 # 186
...
CCAMLR_sum <- CCAMLR_FIS %>% group_by( ASD, season.year, SpeciesCode) %>% summarise ( totC = sum(corr.Catch2) )
...
CCAMLR_c <- left_join (CCAMLR_sum, CCAMLR_spp) # Joining by: "SpeciesCode"
