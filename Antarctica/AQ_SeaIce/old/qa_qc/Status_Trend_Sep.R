for (p in poles){ 
  ...
  r.ice.edge = r.ice
  r.ice.edge[r.ice.edge < 0.10 | r.ice.edge>= 0.50] = NA
  ...
  ###########################################################
  ## select September rasters
  ###########################################################
  SepMonths <- grep("09$", names(s), value=TRUE)
  si <- subset(s, SepMonths)
  ...
  sih = si >=0.10 & si <0.50
  names(sih) <- levels(yrs.s)
  sip = si >=0.15 & r.typ==2
  sip[is.na(r.rgn) | r.rgn==0] = NA
  names(sip) <- levels(yrs.s)  
  ...
  for (yr in range.years){ 
    #yr=2006  ## testing
    ir = which(names(sih) %in% sprintf('X%d',(yr-1):(yr+1))) # collect the layers that include year of interest as well as previous and subsequent year
    lyr = sprintf('avg3yr_%d', yr)
    if (yr==min(range.years)){    
      sih.avg3yr = mean(sih[[ir]])
      sip.avg3yr = mean(sip[[ir]])        
      names(sih.avg3yr) = lyr
      names(sip.avg3yr) = lyr
    } else {
      sih.avg3yr = stack(sih.avg3yr, mean(sih[[ir]]))
      sip.avg3yr = stack(sip.avg3yr, mean(sip[[ir]]))
  ...
  write.csv(z.h.T, sprintf('tmp\\%s_IceEdgeHabitat_sep.csv',p), row.names=FALSE)
