for (p in poles){ 
  ...
  #create an ice edge habitat raster (ice between 10-20%) (used only to visualize):
  r.ice.edge = r.ice
  r.ice.edge[r.ice.edge<0.1 | r.ice.edge>0.2] = NA
  ...
  write.csv(z.h.T, sprintf('tmp\\%s_IceEdgeHabitat_10to20per.csv',p), row.names=FALSE)
