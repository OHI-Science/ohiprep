...
changePlot <- function(repo="~/ohi-global", scenario="eez2013", commit="previous", fileSave){
  ...
  ggsave(file.path(repo, 'figures/DataCheck', paste0(fileSave, "_changePlot_", Sys.Date(), '.png')))
  write.csv(data_new, file.path(repo, 'figures/DataCheck', paste0(fileSave, "_diff_data_", Sys.Date(), '.csv')), row.names=FALSE)
}


scatterPlot <- function(repo="~/ohi-global", scenario="eez2013", commit="previous", goal, fileSave){
  #   scenario <- "eez2013"  ## options: 'eez2012', 'eez2013', 'eez2014', 'eez2015'
  #   commit <- "final_2014"   ## 'final_2014', 'previous', a commit code (ie., 'e30e7a4')
  ...
  ggplot(data_new, aes(x=old_score, y=score)) +
    geom_point(shape=19) +
    theme_bw() + 
    labs(title=paste(goal, scenario, commit, sep=": "), y="New scores", x="Scores from 2014 analysis") +
    geom_abline(slope=1, intercept=0, color="red") +
    geom_text(aes(label=plotLabel), vjust=1.5, size=3)
    
    ggsave(file.path(repo, 'figures/DataCheck', paste0(fileSave, "_scatterPlot_", Sys.Date(), '.png')))
}


goalHistogram <- function(repo="~/ohi-global", scenario="eez2013", goal, fileSave){
  #   scenario <- "eez2013"  ## options: 'eez2012', 'eez2013', 'eez2014', 'eez2015'
  #   fileSave <- 'LSP_trend_data'
  #   goal <- 'LSP'
  ## Useful code: repository(repo)
  ...
  ggplot(data_new, aes(x=score)) +
    ...
    theme_bw() + 
    labs(title=paste(goal, scenario, "2015 analysis", sep=": "), y="Regions", x="Scores") 
  
  ggsave(file.path(repo, 'figures/DataCheck', paste0(fileSave, "_histPlot_", Sys.Date(), '.png')))
}