...
change_plot = function(repo = "ohi-global", scenario="eez2014", commit="previous", 
  ...
  if(save_png){
    ggplot2::ggsave(file.path('changePlot_figures', paste0(fileSave, "_changePlot_", Sys.Date(), '.png')), width=8, height=5)
  }
  
  if(save_csv){
    write.csv(data_new, file.path('changePlot_figures', paste0(fileSave, "_diff_data_", Sys.Date(), '.csv')), row.names=FALSE)
  }
}
