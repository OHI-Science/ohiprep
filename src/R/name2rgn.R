...
name_to_rgn = function(
  ...
  # attach rgn_synonyms; summarize eliminates duplicate rows?
  r = rbind_list(rgns %>% 
  ...
  # ??? are others on there because the "official" OHI name ('eez') is also on the "synonyms" list?
  ...
  # filter to ones with 'eez' or 'ohi_region' in the type.  'eez' is original OHI rgn_name/rgn_id list;
  # 'ohi_region' is from the synonyms.  
  # ??? This cuts out FAO regions (and Antarctic regions)?  (as well as landlocked, disputed, etc)
  # ??? Can we generalize (or create a variation) so 
  # it can also be used for HS and AQ? or is that less important than clarity?
  ...
  # countries in m_r with no match - so tmp_type is NA (this is why left_join() is used).
