...
### ditch the small polys;  smallest EEZ region is Bosnia/Herzegovina at 0.00141 so threshold below that
...
res_list = c(ultralow = 0.5, verylow = 0.25, low = 0.1)
num_poly <- length(rgn_mar@polygons)
for(k in 1:length(res_list)) { # res = c(verylow = 0.5)
  ...
  output_all <- sprintf('rgn_all_gcs_%s_res', names(res_list[k]))
  ...
  output_eez <- sprintf('rgn_eez_gcs_%s_res', names(res_list[k]))
  ...
  output_ant <- sprintf('rgn_ant_gcs_%s_res', names(res_list[k]))
  ...
  output_fao <- sprintf('rgn_fao_gcs_%s_res', names(res_list[k]))
