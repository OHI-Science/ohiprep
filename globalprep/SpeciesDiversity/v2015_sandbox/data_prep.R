    ...
    setup_sh <- function() {
      # mkdirs data tmp
        ...
        if(!dir.exists(dir_tmp)) 
          dir.create(dir_tmp)
        if(!dir.exists(file.path(dir_anx, 'data'))) 
      ...
      # ln -sf $OHI_MODELDIR/GL-NCEAS-OceanRegions_v2013a/data/rgn_offshore_3nm_mol.t* tmp
        ...
        file.symlink(offshore_gcs, dir_tmp)
      ...
      # ln -sf $OHI_MODELDIR/GL-NCEAS-SpeciesDiversity/data/spp.csv tmp/spp_2012.csv
        spp_old <- file.path(dir_rem, 'GL-NCEAS-SpeciesDiversity/ohi_spp/data/spp.csv')
      ...
      # done
        # ??? echo in this context simply outputs the values of these variables to the screen.
