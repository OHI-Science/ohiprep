...
source(file.path(dir_goal, 'lsp_fxn.R'))
...
shp_reorder <- file.path(dir_data_wdpa, 'shps',
                         'WDPA_May2016_shp_ordered')
shp_xformed <- file.path(dir_data_wdpa, 'shps',
...
``` {r chunk out layers for rasterization}
wdpa_poly <- readOGR(dsn   = dirname(shp_xformed),
                     layer = basename(shp_xformed))
quantile(wdpa_poly@data$STATUS_YR, probs = seq(0, 1, 0.1))

break_list <- c(0, 1968, 1983, 1990, 1995, 1999, 2000:2015)
break_poly <- function(poly, yr0 = 0, yr1 = 2015) {
  if(length(yr0) == 0) yr0 <- 0 ### to allow for indexing to [0] in a vector
  else yr0 <- yr0 + 1
  message('yr0 = ', yr0, '; yr1 = ', yr1)
  poly1 <- poly[(poly@data$STATUS_YR >= yr0 & poly@data$STATUS_YR <= yr1), ]
}
...
foreach(i = c(1:length(break_list))) %dopar% { # i = 1
  wdpa_poly_sub <- break_poly(wdpa_poly, break_list[i - 1], break_list[i])
  layername <- paste0(basename(shp_xformed), '_yr', break_list[i])
  writeOGR(wdpa_poly_sub, 
           dsn    = dirname(shp_xformed),
           layer  = layername,
           driver = 'ESRI Shapefile')
}
...
``` {r rasterize_wdpa_subsets}
shpnames <- paste0(shp_xformed, '_yr', break_list, '.shp')
tifnames <- file.path(dir_goal_anx, 'rasts', paste0(basename(shp_xformed), '_yr', break_list, '.tif'))
file.exists(shpnames)

### Check base raster:
dir_rast <- file.path(dir_anx, 'spatial/v2015/data/rgn_raster_500m')
...
if(!file.exists(out_tif_tmp))
  file.copy(fn_rast_base, out_tif_tmp)
...
message('elapsed gdal_rasterize: ', (proc.time() - ptm)[3])
### rewrite for compression and unlink the temp raster
message('Compressing: \n  ', out_tif_tmp, '\nto: \n  ', out_tif)
raster::writeRaster(rast_tmp, out_tif, overwrite = TRUE)
message('elapsed gdal_rasterize + writeRaster: ', (proc.time() - ptm)[3])
# unlink(out_tif_tmp)
...
in_shp <- file.path(dir_data_wdpa, 
                         'WDPA_May2016_shp_xformed.shp')
