...
prot_1km <- stats_1km %>%
  ...
  mutate(pct_prot   = n_cells_cum / n_cells_tot,
         lsp_status = ifelse(pct_prot > lsp_thresh, 100, (pct_prot / lsp_thresh) * 100))
...
prot_df <- prot_1km %>%
  dplyr::select(rgn_id, year, lsp_st_1km = lsp_status) %>%
  left_join(prot_1km %>% 
              dplyr::select(rgn_id, year, lsp_st_3nm = lsp_status),
            by = c('rgn_id', 'year')) %>%
...
lsp_v2015 <- read_csv('~/github/ohi-global/eez2013/scores.csv') %>%
  ...
  left_join(prot_df %>% 
              rename(lsp_v2016 = lsp_status) %>%
              filter(year == 2012), 
            by = 'rgn_id') %>%
  dplyr::select(rgn_id, lsp_v2015, lsp_v2016) %>%
  left_join(rgn_names, by = 'rgn_id')

library(ggplot2)
lsp_plot_v2015 <- ggplot(lsp_v2015, 
                        aes(x = lsp_v2015, y = lsp_v2016)) +
...
lsp_v2015 <- lsp_v2015 %>%
  mutate(diff = lsp_v2016 - lsp_v2015) %>%
  arrange(diff)

write_csv(lsp_v2015, file.path(dir_goal, 'int/check_vs_2015.csv'))
```
