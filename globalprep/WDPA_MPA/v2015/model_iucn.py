...
# outputs
ply_d = '%s/poly_wdpa_d' % gdb    # designated
ply_p = '%s/poly_wdpa_p' % gdb    # protected, within designated
...
arcpy.Select_analysis(poly_wdpa, ply_d, '"STATUS"=\'Designated\'')

print 'Creating prot_status field to flag (STATUS = Designated) and [(Int?l/Regional) or (no take) or (IUCN cat I ? IV)]'
arcpy.AddField_management(ply_d, 'prot_status', 'SHORT')
code_block = """
def get_prot(typ, iucn, notake):
    if (typ in ['International', 'Regional']): 
      protected = 1   
    elif (notake == 'All'): 
      protected = 1
    else: 
      iucn_good = {'Ia': 1, 'Ib': 1, 'II': 2, 'III': 3, 'IV': 4}
      protected = iucn in iucn_good
    return(protected)
"""
arcpy.CalculateField_management(ply_d, 'prot_status', 'get_prot(typ=!DESIG_TYPE!, iucn=!IUCN_CAT!, notake=!NO_TAKE!)', 'PYTHON_9.3', code_block)

# select only prot_status = 1
print 'Selecting only prot_status = 1 features and creating new feature class'
arcpy.Select_analysis(ply_d, ply_p, '"prot_status" = 1')
...
arcpy.AddField_management(ply_p, 'poly_priority', 'FLOAT')
arcpy.CalculateField_management(ply_p, 'poly_priority', '1/(!STATUS_YR! + 1)', 'PYTHON_9.3')

# polygon to raster
print 'From Protected polygons, create raster .tif'
arcpy.PolygonToRaster_conversion(ply_p, 'STATUS_YR', tif, 'MAXIMUM_COMBINED_AREA', 'poly_priority', msk)
