...
**move to methods or delete if redundant**
and rescaled values greater than 1 based on how close the value is to the threshold of 1. This was done by using the following equation:
...
file <- list.files(file.path(raw_dir,'WHOI/annual'),pattern="2016.nc",recursive=T,full.names = T) #grab only 2016
...
This has already been done for years 2005 - 2015 for the OHI 2016 assessment. Here we create the same rasters for 2015-2020 for this years and future OHI global assessments.
...
#annual_arag is a function that calculates annual average surface aragonite saturation 
  ...
  annual_arag<-function(file){
    ...
    year = substr(file,nchar(file)-6,nchar(file)-3)
...
# land cells are given large, strange value, so set these to NA
  ...
  writeRaster(out_moll,filename = paste0(file.path(oagit_dir,"v2017/int/global_arag_avg_moll_"),year), format='GTiff', overwrite=T)
    
}

#run the function for each file in the file list using lapply

lapply(file,annual_arag)
```
...
r_2016 <- raster(file.path(oagit_dir,'v2017/int/global_arag_avg_moll_2016.tif'))
...
oaRescale <- function(file){
  ...
  mean[mean<0]<-0                                  #all values less than 0 (indicating a decrease in acidity) are capped at 0
    ...
    writeRaster(mean,filename=paste0(oagit_dir,'/v2017/int/oa_rescaled_',yr,sep=""),format='GTiff',overwrite=T)

}

file = file.path(oagit_dir,'v2017/int/global_arag_avg_moll_2016.tif')
...
Since there are oceanic cells with no information in the raw data, we need to fill in these gaps. We do this by interpolating across the globe using the data we have with an Inverse Distance Weighting (IDW) function.


```{r interpolate}

library(gstat)
r  <- raster(file.path(oagit_dir,'v2017/int/oa_rescaled_2016.tif')) #oa raster
...
plot(z,col=cols,box=F,axes=F,main='Rescaled and Interpolated Ωaragonite layer for 2016')
...
resample(z,ocean,filename = paste0(oagit_dir,'/v2017/int/annual_oa_rescaled_int_1km_2016.tif'),progress='text')
...
```{r mask,eval=F}
#bring in resampled raster
oa <- raster(file.path(oagit_dir,'v2017/int/annual_oa_rescaled_int_1km_2016.tif'))%>%
      mask(ocean,filename = file.path(oagit_dir,'v2017/output/oa_prs_layer_2016.tif',overwrite=T))
```

```{r plot_final}
plot(oa,box=F,axes=F,col=cols,main='Final Ocean Acidification \nPressure Layer 2016')
