...
# read in 1958-2016 data
      ...
      raw = nc_open(file.path(raw_dir,'WHOI/cesm_co2sys_1958-2016.1x1d.nc'))
        print(raw)
        
      raw <- stack(file.path(raw_dir,'WHOI/cesm_co2sys_1958-2016.1x1d.nc'), varname="OARG") #grab aragonite variable
...
foreach (i = seq(1,708,by=12)) %dopar% {
  ...
  yr = substr(names(raw_brick[[i]]),2,5)
...
oaRescale <- function(file){
  ...
  mean[mean<0]<-0                                  #all values less than 0 (indicating a decrease in acidity) are capped at 0
    ...
    writeRaster(mean,filename=paste0(oagit_dir,'/v2017/int/annual_avg_moll_rescaled/oa_rescaled_',yr,sep=""),format='GTiff',overwrite=T)
}

files = list.files(file.path(oagit_dir,'v2017/int/annual_avg_moll'),full.names=T)
...
files <- list.files(file.path(oagit_dir,'v2017/int/annual_avg_moll_rescaled'),full.names=T)
...
plot(raster(file.path(oagit_dir,'v2017/int/annual_avg_moll_rescaled_int/oa_resc_int_2016.tif')),col=cols,box=F,axes=F,main='Rescaled and Interpolated Ωaragonite layer for 2016')
