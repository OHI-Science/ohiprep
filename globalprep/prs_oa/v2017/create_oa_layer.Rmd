...
## Calculate annual mean aragonite saturation state for 2016
...
This has already been done for years 2005 - 2015 for the OHI 2016 assessment. Here we do the same for 2016 to include in the OHI 2017 global assessment.
...
resample(z,ocean,filename = paste0(oagit_dir,'/v2017/int/annual_oa_rescaled_int_1km_2016.tif'),progress='text',overwrite=T)
...
Now that all cells are at a ~1km^2 resolution, and rescaled between 0 and 1 we want to remove the land and extra cell values. We can use the `mask` function from the `raster` package to do this using our ocean raster.
