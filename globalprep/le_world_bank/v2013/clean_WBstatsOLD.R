...
uifilesave = paste(dir1, 'data/', 'GL-WorldBank-Statistics_v2012-cleaned.csv', sep='')
...
for(k in 1:length(layer_uni)) { # k=1
  ...
  # save 2013a files
  layersave = paste(dir1, 'data/', layernames[k], sep='') 
  cleaned_layer$rgn_nam = NULL
  
  #   if(layer_uni[k] == 'UEM'){
  #     cleaned_layer = cleaned_layer[cleaned_layer[,1] != 5,] # New Caledonia only has 1 year w data; remove it so geogapfilled (it will crash temporal gapfilling)
  #   }
  
  #   write.csv(cleaned_layer, paste(dir1, 'data/', 'rgn_wb_gdp_2013a_tmp.csv', sep=''), na = '', row.names=FALSE) # debugging
  #   x = read.csv('/Volumes/data_edit/model/GL-WorldBank-Statistics_v2012/data/rgn_wb_gdp_2013a_tmp.csv')
  cleaned_layert = temporal.gapfill(cleaned_layer, fld.id = 'rgn_id', fld.value = names(cleaned_layer)[2], fld.year = 'year', verbose=F); head(cleaned_layert,55) 
...
## Sept 22: do LAB/POP calcs; if LAB > POP, fix.
library(zoo)
fl = read.csv(file.path(dir1, 'data', 'rgn_wb_lab_2013a.csv')); head(fl)
names(fl)[2] = 'labcount'
fp = read.csv(file.path(dir1, 'data', 'rgn_wb_pop_2013a_updated.csv')); head(fp)
names(fp)[2] = 'popcount'

fm = merge(x=fl,  by.x = c('rgn_id', 'year'),
           y = fp,
           all.x = T); head(fm,50)
 
fm2 = fm; head(fm2,30)
# learn how: fm2$popcount = ddply(fm2, .(popcount), .fun = na.locf); head(fm2,30)
runi=unique(fm2$rgn_id)
for(i in runi) { # i = 1
  
  # same rgn_id
  idex = which(i == fm2$rgn_id)
  
  # same rgn_id & not NA
  ndex = which(i == fm2$rgn_id & !is.na(fm2$popcount))
  
  # can't have first value be NA
  fm2$popcount[idex][1] = fm2$popcount[ndex][1]
  
  #carry forward
  fm2$popcount[idex] = na.locf(fm2$popcount[idex])
}
head(fm2,300)

#ratio calc
fm2$ratio = fm2$labcount / fm2$popcount; tail(fm2,60)
sum(fm2$ratio > 1, na.rm=T)
unique(fm2$rgn_id[fm2$ratio > 1])

rgn_georegions = read.csv('/Volumes/data_edit/model/GL-NCEAS-OceanRegions_v2013a/manual_output/rgn_georegions_wide_2013b.csv'); head(rgn_georegions)
fm3 = merge(x=fm2,  by.x = c('rgn_id'),
           y = rgn_georegions,
           all.x = T); head(fm3)
...
for(r in r2uni){ # r = 29
  ...
  # same r2 and good ratio
  gdex = which(fm3$r2 == r & fm3$ratio < 1)
  
  # mean ratio of those with appropriate ratio
  ratmean = mean(fm3$ratio[gdex])
 
  # same r2 and bad ratio
  bdex = which(fm3$r2 == r & fm3$ratio >= 1)
  
#   if (!exists('ratiofilled')){ # identify the ones processed--why didn't this work for hours. TODO: fix
#     ratiofilled = as.numeric(unique(fm3$rgn_id[bdex]))
#   } else {
#     ratiofilled = rbind(ratiofilled,  as.numeric(unique(fm3$rgn_id[bdex])))
#   }
    
  # sub ratmean for bdex value
  fm3$ratio[bdex] = ratmean  
}

ratiofilled = ratiofilled[-1,]
ratiofilled = unique(ratiofilled)

# deal with unpopulated
fm3$popcount = as.numeric(fm3$popcount)
fm3$ratio[fm3$popcount < 1] = 0; head(fm3)
names(fm3)[5] = 'ratio_labor_totalpop'; head(fm3)

fm4 = fm3[, c('rgn_id', 'year', 'ratio_labor_totalpop')]; head(fm4)
summary(fm4)
fm4[is.na(fm4$ratio_labor_totalpop),] # southern islands

# save 2013a:
write.csv(fm4, file.path(dir1, 'data', 'rgn_wb_ratio_lab_pop_2013a.csv'), na = '', row.names=FALSE)
...
for(i in runi){ # i = 1
  ...
  # same rgn_id
  idex = which(i == fm5$rgn_id)
  
  # max year
  y = max(fm5$year[idex])
  
  sdex = which(i == fm5$rgn_id & fm5$year == y)
  
  fm5 = fm5[-sdex , ]
  
}
head(fm5, 30)
write.csv(fm5, file.path(dir1, 'data', 'rgn_wb_ratio_lab_pop_2012a.csv'), na = '', row.names=FALSE)




# Sept 10: change gdp, uem, ppp, lab from rgn to country
...
## 
# convert to country_2012 for wages multipliers
library(plyr)
...
# Sept 12: Make Employment Data File ----
