...
write.csv(b_bmsy_data, "globalprep/SAUP_FIS/tmp/b_bmsy_scores_constrained.csv", row.names=FALSE)
...
#checks:
length(unique(b_bmsy_data_uniform$stock_id))  #should be N=2706
...
write.csv(b_bmsy_data_uniform, "globalprep/SAUP_FIS/tmp/b_bmsy_scores_uniform.csv", row.names=FALSE)
...
names <- read.csv(file.path(dir_neptune_data, 'git-annex/globalprep/SAUP_data_2015/raw/ohi_taxon.csv'))
...
ggsave(width=17, height=10, units="in", 
       sprintf("globalprep/SAUP_FIS/tmp/bbmsy_figs/Bmsy_vs_catch_%s_%s.png", small[i], large[i]))
}