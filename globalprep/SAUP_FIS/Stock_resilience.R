...
newSAUP = read.csv(file.path(dir_neptune_data, 'git-annex/globalprep/SAUP_data_2015/tmp/Catch_Value_11062015_summary.csv')) %>%
  select(saup_id = EEZID, fao_id=FAOAreaID, yr=Year, Taxonkey=TaxonKey, tonnes=catch); head(newSAUP)
head(newSAUP); dim(newSAUP)

# filter species and 1980 or greater
newSAUP <- newSAUP %>%
  #  left_join(tax) %>%   # add species names # Joining by: "Taxonkey"
  mutate(stock_id = paste(Taxonkey, fao_id, sep='_')) %>% 
  mutate(TL = substr(Taxonkey, 1, 1)) %>%
  filter(TL == 6, yr >= 1980) %>%
  filter(saup_id != 274) #remove Gaza Strip
head(newSAUP); dim(newSAUP)

# filter stocks with at least 10 years of non-0 data within a taxon/fao_region
newSAUP_10year <- newSAUP %>% 
#   group_by(fao_id, Taxonkey, stock_id, yr) %>% # removing the saup id
#   mutate(t_Catch = sum(tonnes)) %>% 
  group_by(fao_id, Taxonkey) %>%
  mutate(N = length(unique(yr))) %>%
  filter (N > 9) %>% # at least 10 non-0 data points
  select(stock_id, fao_id, Taxonkey, saup_id, yr, tonnes)
  ungroup() 
head(newSAUP_10year); dim(newSAUP_10year); summary(newSAUP_10year)

length(unique(newSAUP_10year$stock_id))-length(unique(newSAUP$stock_id)) # excludes -396 stocks
# newSAUP_10year[newSAUP_10year$stock_id=='600033_21', ] # good one to check at each step to understand process, eventually gets cut
# data.frame(newSAUP_10year[newSAUP_10year$stock_id=='600024_21', ]) # data starts at 1980, no gaps
# data.frame(newSAUP_10year[newSAUP_10year$stock_id=='601077_57', ]) # data starts in 2001, some gaps


#################################
## Adding in zero catch data
#################################
# get first year of catch data within each saup/fao/taxon
newSAUP_10year <- newSAUP_10year %>% 
  group_by(fao_id, saup_id, Taxonkey) %>%
  mutate(Year1 = min(yr)) %>%
ungroup() 

head(newSAUP_10year); dim(newSAUP_10year); summary(newSAUP_10year)
data.frame(newSAUP_10year[newSAUP_10year$stock_id=='600752_51', ])

newSAUP_10year_wide <- spread(newSAUP_10year, yr, tonnes)
newSAUP_10year_wide[is.na(newSAUP_10year_wide)] <- 0 

newSAUP_10year_gather <- gather(newSAUP_10year_wide, "year", "tonnes", which(names(newSAUP_10year_wide)==1980):which(names(newSAUP_10year_wide)==2010))

newSAUP_10year_gather <- newSAUP_10year_gather %>%
  mutate(year = as.numeric(as.character(year))) %>%
  filter(year >= Year1) %>%
  select(stock_id, fao_id, Taxonkey, saup_id, year, tonnes)
data.frame(newSAUP_10year_gather[newSAUP_10year_gather$stock_id=='600752_51', ])

#########################################
## Calculating mean and relative catch throughout the time series (1980-2011) per stock
#########################################
CatchData <- newSAUP_10year_gather %>% 
  group_by(stock_id, fao_id, Taxonkey, saup_id) %>% 
  summarize(av_ct = mean(tonnes, na.rm=T)) %>%
  ungroup()
data.frame(newSAUP_meanC[newSAUP_meanC$stock_id=='600752_51' & newSAUP_meanC$saup_id=='356', ]) #should all have the same mean count

CatchData <- CatchData %>%
  mutate(stock_saup_id = paste(stock_id, saup_id, sep="_")) %>%
  group_by(Taxonkey, fao_id) %>%
  mutate (rel_ct = av_ct/sum(av_ct)) %>%
  ungroup()

sum(CatchData$rel_ct)
length(unique(CatchData$stock_id))
...
eez_r <- eez_r %>%
  ...
  group_by(saup_id) %>%
  summarize(Score = mean(Score))   #N=279, average score for cases where several ohi regions are represented by one saup region
head(eez_r)
eez_r[duplicated(eez_r$saup_id),]  #should be no duplicates

res_eez <- CatchData %>%  # N=15960
  filter(saup_id > 0) %>%
  left_join(eez_r) %>%
...
tax <- read.csv(file.path(dir_neptune_data, 'git-annex/globalprep/SAUP_data_2015/raw/ohi_taxon.csv')) %>%
...
rfmo_fao <- gather(rfmo_fao, key = rfmo, value = proparea, -rgn_id, -rgn_name, -total, -fao_id) ; head(res_hs)
...
#     tmp2[tmp2$stock_saup_id=="600094_47_0", ]
  mutate(part_score = rel_ct * proparea * Score) %>%
  group_by(stock_saup_id, stock_id, Taxonkey, fao_id, saup_id) %>%
  summarize(part_score = max(part_score, na.rm=TRUE)) %>%
  ungroup()
...
write.csv(FinalRes, 'globalprep/SAUP_FIS/tmp/stock_resil_06cutoff_2015.csv', row.names=FALSE)
