...
sla_baseline <-rbrick_sla[[1:base_yrs]] %>%
  calc(mean,na.rm=T,filename = file.path(dir_M,'git-annex/globalprep/prs_slr/v2016/int/sla_baseline.tif'))
...
foreach(i = 1:nlayers(sla_delta))%dopar%{
  ...
  r <- sla_delta[[i]]%>%
        projectRaster(ocean,filename = paste0(file.path(dir_M,'git-annex/globalprep/prs_slr/v2016/int/sla_delta_moll/sla_delta_moll_'),
