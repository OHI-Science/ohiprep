...
mclapply(nc_files[273:276],months_coast,mc.cores=14)
...
s <- list.files(file.path(dir_git,'globalprep/prs_slr/v2016/int/sla_rate_5yrs'),full.names=T)%>%stack()

# make a time variable (to be used in regression)
time <- 1:nlayers(s)

slr_rate <- calc(s,fun=function(x){ if (is.na(x)){ NA } else {lm(x ~ time)$coefficients[2] }})
...
The reference point is `r ref*0.01` mm per year.
...
mclapply(annual_means,rescale_slr,mc.cores=14)
