...
## format data to have complete years/regions and convert percentage of jobs to proportion of jobs
    rgn_names <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/eez/spatial/rgns_list.csv', stringsAsFactors = FALSE) %>%
    dplyr::select(rgn_id)
...
#  filter(!is.na(Ep)) %>%
  select(rgn_id, S_score=S_score_2)
summary(tr_sust)
