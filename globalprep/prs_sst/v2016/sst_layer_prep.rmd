...
```
  ...
  out = projectRaster(diff,crs=mollCRS,progress='text',over=T)%>%
         resample(.,ocean,method='ngb',progress='text')%>%
          mask(.,ocean,filename=paste0(dir_M,'git-annex/globalprep/prs_sst/v2015/output/sst_',min(yrs),'_',max(yrs),'-1985_1989.tif',sep=""),overwrite=T)
  ...
  out_rescale = calc(out,fun=function(x){ifelse(x>0,ifelse(x>ref,1,x/ref),0)},progress='text',
                     filename=paste0(dir_M,'git-annex/globalprep/prs_sst/v2015/output/sst_',min(yrs),'_',max(yrs),'-1985_1989_rescaled.tif',sep=""),overwrite=T)
}
```
...
```{r, message=F,warning=F, eval=F}    
  ...
  rast_2012 <- raster(file.path(dir_M, 'git-annex/globalprep/prs_sst/v2015/output/sst_2005_2009-1985_1989.tif'))
  quantile(rast_2012,prob=0.9999)  #118.4027
  rast_2013 <- raster(file.path(dir_M, 'git-annex/globalprep/prs_sst/v2015/output/sst_2006_2010-1985_1989.tif'))
  quantile(rast_2013,prob=0.9999)   #133.0371 
  rast_2014 <- raster(file.path(dir_M, 'git-annex/globalprep/prs_sst/v2015/output/sst_2007_2011-1985_1989.tif'))
  quantile(rast_2014,prob=0.9999)  #127.1995
  rast_2015 <- raster(file.path(dir_M, 'git-annex/globalprep/prs_sst/v2015/output/sst_2008_2012-1985_1989.tif'))
...
```{r,message=F,warning=F, eval=F}
  ...
  out <- raster(file.path(dir_Meptune_data, 
                          sprintf('git-annex/globalprep/prs_sst/v2015/output/sst_%s_%s-1985_1989.tif', i, final_yr)))  
  out_rescale = calc(out,fun=function(x){ifelse(x>0,ifelse(x>ref,1,x/ref),0)},progress='text',
                     filename=paste0(dir_M,'git-annex/globalprep/prs_sst/v2015/output/sst_',i,'_',final_yr,'-1985_1989_rescaled_v2.tif',sep=""),overwrite=T)
  }
```
