---
title: "Ocean Acidification pressure procedure"
...
`create_global_oa_pressures_layer` takes prepped Ocean Acidification input raster layers (created by oa_dataprep.R) and does the following:
...
#set root directory
    ...
    opts_knit$set(root.dir = file.path(dir_N,'git-annex/globalprep/Pressures_acid/v2015'),warning=FALSE, message=FALSE)
...
plot('working/annualchange_2005-2014/difference_from_hist_mean_2014.tif',col=cols,main='Change in aragonite saturation state\n from 1880s to 2014')
```
***
...
```{r}
  ...
  plot('working/annual_oa_rescaled/oa_rescaled_2014.tif',col=cols,main='Rescaled data for 2014')
```
***  
...
```{r}
 ...
 # sapply(rescaled,resample)
  ...
  plot('working/annual_oa_rescaled_1km_2014.tif',col=cols,main='Rescaled to 1km (2014)')
...
plot('working/annual_oa_rescaled_1km_int_2014.tif',col=cols,main='Interpolation to fill in NAs')
```
***
...
plot('output/annual_oa_rescaled_1km_int_clip_2014.tif',col=cols,main='Final Ocean Acidification\n Pressure Layer for 2014')
```
***
