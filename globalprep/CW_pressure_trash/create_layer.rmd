...
```{r unlog, message=F,warning=F,echo=F,eval=F}
...
## Step Three: Stack all rasters within weight and count
...
## Step Four: Sum across 4 datasets
```{r sum, message=F,warning=F,echo=F}
    w_sum = calc(weight,fun=sum)
    c_sum = calc(count,fun=sum)
```

## Step Five: Reproject to mollweide
```{r reproject, message=F, warning=F,echo=F, eval=F}
...
## Step Six: Resample to 1km 
```{r resample, message=F,warning=F,echo=F,eval=F}
...
## Step Seven: Read in mollweide rasters
...
## Step Eleven: rescale to 99.99th quantile
...
## Step Twelve: Compare count vs weight
