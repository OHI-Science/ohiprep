...
data = saup_1999to2003


ggplot(data,aes(x=log(avg_catch_1999to2003),y=log(yrs_1999to2003)))+
  geom_point(shape=19)+
  theme_bw() +
...
sub = subs(old_saup_eez,saup_06_10,by='old_saup_id',which='pct_chg',subsWithNA=TRUE,progress='text',filename='v2015/pct_change_rasters/pct_chg_99to03_06to10.tif')

chg = raster('v2015/pct_change_rasters/pct_chg_99to03_06to10.tif')

# Bring in catch data (not standardized by productivity)

dem_d = raster(file.path(saup_pressures,'catch_dem_d_gcs.tif'))

# reproject to mollweide

projection(dem_d)<- "+proj=longlat +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +no_defs"

moll_crs = CRS("+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_defs")

dem_d_moll = projectRaster(dem_d,crs=moll_crs,over=T)

#resample

dem_d_moll_1km = resample(dem_d_moll,chg,progress='text',method='ngb',filename='v2015/orig_catch_pressures/dem_d_1km_moll.tif',overwrite=T)
