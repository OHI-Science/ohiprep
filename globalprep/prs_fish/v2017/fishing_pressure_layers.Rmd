...
foreach(yr = years) %dopar%{ #yr = 2014
  ...
  high <- raw %>%
          ...
          rowwise() %>%
          dplyr::mutate(catch = sum(LSF_CR, IUU_CR, Discards_CR, na.rm=TRUE) * OceanArea) %>% #there shouldnt be NAs but just in case
          dplyr::group_by(Seq) %>%
...
raster::subs(saup_rast, high, by = 1, which = 2, subsWithNA=TRUE, filename = paste0('int/high_bycatch/annual_catch/high_bc_',yr,'.tif'), overwrite=TRUE) 
  ...
  low <- raw%>%
          filter(bycatch == "low")%>%
          rowwise()%>%
          mutate(catch = sum(LSF_CR, IUU_CR, Discards_CR, na.rm=TRUE) * OceanArea)%>% #there shouldnt be NAs but just in case
          group_by(Seq)%>%
...
raster::subs(saup_rast, low, by = 1, which = 2, subsWithNA=TRUE, filename = paste0('int/low_bycatch/annual_catch/low_bc_',yr,'.tif'),overwrite=T) 
  ...
  artisanal <- raw%>%
               group_by(Seq)%>%
              summarise(cell_catch = sum(SSF_CR * OceanArea)) #small scale fisheries catch rate
