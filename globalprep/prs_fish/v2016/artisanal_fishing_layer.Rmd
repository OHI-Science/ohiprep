...
foreach (yr = c(2003:2010)) %dopar%{
  ...
  overlay(art,npp,fun=function(x,y){x/y},filename = paste0('int/artisanal_npp/artisanal_npp_',yr,'.tif'))  
...
```{r plot_art_npp}
plot(ocean, col='cornsilk2', axes=F, main='Catch (tons) standardized by NPP \n 2010', legend=F) 
plot(raster('int/artisanal_npp/artisanal_npp_2010.tif'),col=cols,axes=F,add=T)
...
max <- log(max(vals,na.rm=T)+1) #11.73944
