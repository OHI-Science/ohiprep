...
saup_area <- area(raster(hb_files[1]))
...
foreach (yr = c(2003:2010)) %dopar%{
  ...
  cat_hb <- hb_files[substr(hb_files,90,93) == yr]%>% 
              raster()%>%
              overlay(.,saup_area,fun=function(x,y){(x/y)*0.8732508})%>% #0.87325 km2 is the actual size of our cells, not 1km2
              projectRaster(crs = mollCRS,over=T,method = 'ngb')%>%
              resample(.,ocean,method = 'ngb', filename = paste0(dir_git,'/int/annual_hb_catch_1km/annual_hb_catch_1km_',yr,'.tif'))                      
  
  cat_lb <- lb_files[substr(lb_files,90,93) == yr]%>%
              raster()%>%
              overlay(.,saup_area,fun=function(x,y){(x/y)*0.8732508})%>%
              projectRaster(crs = mollCRS,over=T,method = 'ngb')%>%
              resample(.,ocean,method = 'ngb', filename = paste0(dir_git,'/int/annual_lb_catch_1km/annual_lb_catch_1km_',yr,'.tif'))
...
max_v_hb <- max(vals,na.rm=T) # 31.95568

ref_hb <- log(quantile(vals,prob = 0.9999,na.rm=T)+1) #2.601586
...
min_v_lb <- min(lb_vals,na.rm=T) 
max_v_lb <- max(lb_vals,na.rm=T) 


ref_lb <- log(quantile(lb_vals,prob = 0.9999,na.rm=T)+1) #3.10426
...
foreach (i = 2003:2006) %dopar%{
  ...
  out_hb <- mean_hb[which(substr(mean_hb,90,93) == i)]%>%
            raster()%>%
            calc(fun=function(x){ifelse(x>ref_hb,1,x/ref_hb)},filename = paste0(file.path(dir_git),'/out/high_bycatch/hb_fish_pressure_',yrs[1],'-',yrs[5],'.tif'),overwrite=T)
  
  out_lb <- mean_lb[which(substr(mean_lb,90,93) == i)]%>%
            raster()%>%
            calc(fun=function(x){ifelse(x>ref_lb,1,x/ref_lb)},filename = paste0(file.path(dir_git),'/out/low_bycatch/lb_fish_pressure_',yrs[1],'-',yrs[5],'.tif'),overwrite=T)
