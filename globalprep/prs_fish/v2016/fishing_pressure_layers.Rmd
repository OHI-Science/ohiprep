...
plot(raster(npp[13]),col=cols,axes=F,main = 'Net Primary Production (for)mg C/m2/day) \n 2015')
```
...
foreach (yr = c(2003:2010)) %dopar%{
  ...
  #testing the area division/reprojection
#  ext <- extent(-39,8.7,4.2,56)
#   a   <- crop(saup_area,ext)%>%calc(.,fun=function(x){x/0.87325})
# 
#   a_proj <- projectRaster(a,crs=mollCRS,over=T,method='ngb')
# 
#   orig <-  hb_files[substr(hb_files,90,93) == yr]%>%
#               raster()%>%
#               crop(ext)
# 
#  t <- orig%>%projectRaster(crs=mollCRS,over=T,method='ngb')
# 
#   test <- orig%>%
#               overlay(.,a,fun=function(x,y){(x/y)*0.8732508})%>%
#               projectRaster(crs = mollCRS,over=T,method='ngb')%>%
#               resample(.,ocean, method= 'ngb')
# 
#   test_2 <- orig%>%
#             projectRaster(crs = mollCRS, over=T, method = 'ngb')%>%
#             overlay(.,a_proj,fun=function(x,y){(x/y)*0.87})%>%
#             resample(.,ocean,method='ngb')
#   
#   test_3 <- orig%>%
#             overlay(.,a,fun=function(x,y){x/y})%>%
#             projectRaster(crs=mollCRS, over=T,method='ngb')%>%
#             resample(.,ocean,method='ngb')
# 
# 
#   orig_vals <- getValues(orig)%>%sum(.,na.rm=T) #4,886,840
# 
# orig_vals_2 <- getValues(t)%>%sum(na.rm=T)
#  
#    new_vals <- getValues(test)%>%sum(na.rm=T) #4,968,374
#  
#    new_vals_2 <- getValues(test_2)%>%sum(na.rm=T) #same as new_vals - 4,968,374
#  
  #diff is 81,534
```
