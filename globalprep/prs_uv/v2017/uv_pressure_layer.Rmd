...
#NOTE:
As of 2-15-2017, only the first few chunks have been run to get annual rasters created for use in the Northeast.
...
2. For each week/year/cell, calculate the mean and sd, so each cell would have ~624 (12*52) values (2005-2016)  
...
5. Calculate the total number of anomalies in the reference period (in our case, 2005-2009, for a total of 52*5 anomalies per cell)  
...
For **OHI 2017** we are using a higher resolution dataset from the same satellite.
...
files = list.files(file.path(raw_data_dir,'data'),pattern='*.he5$',full.names=T)

attribute <- "/HDFEOS/GRIDS/OMI UVB Product/Data Fields/ErythemalDailyDose"
qual      <- "/HDFEOS/GRIDS/OMI UVB Product/Data Fields/OMUVBQuality"

#function to turn HDF file into a raster 

#each file has 15 scenarios (15 orbits). After removing the fill value, we average across these orbits to get a single daily raster
he_to_ras <- function(x){
    ...
    for(k in 1:15){ 
        ...
        r <- raster(h[,,k],xmn=-90,ymn=-180,xmx = 90, ymx = 180, crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")%>%
                t()%>%
                flip(direction='y')
        r[r== -1267650600228229401496703205376]<-NA #The NA value is 
        s <- stack(s,r)
        
    }
    
    m <- calc(s,fun=function(x){mean(x,na.rm=T)})
    return(m)
  ...
  print(i)
    ...
    for (j in 1:length(days)){
      ...
      week_mean    <-  calc(uv_week, fun=function(x){mean(x,na.rm=T)},
                         filename=paste0(file.path(dir_M),'/git-annex/globalprep/prs_uv/v2017/int/weekly_means/weekly_means_',i,'_',j,'.tif'),
                         overwrite=T)
      
      
      week_sd      <-  calc(uv_week, fun=function(x){sd(x,na.rm=T)},
                         filename=paste0(file.path(dir_M),'/git-annex/globalprep/prs_uv/v2017/int/weekly_sd/weekly_sd_',i,'_',j,'.tif'),
                         overwrite=T)
      
      week_mean_sd <-  overlay(week_mean,week_sd,fun=function(x,y){x+y},
                         filename=paste0(file.path(dir_M),'/git-annex/globalprep/prs_uv/v2017/int/weekly_mean_sd/weekly_mean_sd_',i,'_',j,'.tif'),
                         overwrite=T)
...
for (i in 2005:2016){
  ...
  year = calc(s,fun=function(x){sum(x,na.rm=T)},progress='text',filename=paste0(file.path(dir_M),'/git-annex/globalprep/prs_uv/v2017/int/annual_anoms/annual_pos_anomalies_',i,'.tif'),overwrite=T)
}
...
l   <- list.files(file.path(dir_M,'git-annex/globalprep/prs_uv/v2017/int/annual_anoms/'),full.names=T)
ref_files <- c(grep("2005", l), grep("2006", l), grep("2007", l), grep("2008", l), grep("2009", l))

ref <- l[ref_files]%>%stack()%>%
...
foreach(i = c(2006:2011)) %dopar% { #i=2005

  yrs <- c(i:(i+4))
  
  s   <- stack(l[substr(l,91,94)%in%yrs])%>%sum(.,na.rm=T)
...
##Feb 21 2017 JA stopped here and did UV for northeast. Need to update the following for global 2017
...
out <- list.files(file.path(dir_M,'git-annex/globalprep/prs_uv/v2017/output'),full.names=T)[5]%>%raster()
