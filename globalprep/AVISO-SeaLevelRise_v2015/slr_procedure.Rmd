...
opts_knit$set(root.dir = file.path(dir_N,'git-annex/globalprep/AVISO-SeaLevelRise_v2015'))
```
...
Original resolution of the data is X. To resample the data to 1km resolution, a template ocean raster that is at the desired resolution (~1km) and contains all cells included within OHI regions, is used.
```{r,message=FALSE,warning=FALSE}
...
All values at or above the 99.99th quantile are set to equal 1.
```{r,warning=FALSE,message=FALSE}
...
Interpolating the data was done using ArcGIS. All NA cells were interpolated using nearest neighbor. The python script can be found [here](https://github.com/OHI-Science/ohiprep/blob/master/globalprep/AVISO-SeaLevelRise_v2015/scripts/SLR_interpolation.py)  
```{r,message=FALSE,warning=FALSE}
