...
if(!file.exists(spp_info_from_api_file)) {
...
From the habitats gleaned in the previous chunk, generate an inclusion list based on those that are considered marine.
...
write_csv(spp_mar_list_clean, file.path(dir_goal, 'int/spp_marine_all.csv'))

### x <- read_csv(file.path(dir_goal, 'int/spp_marine_all.csv'))
```


`r file.path(dir_goal, 'int/spp_marine_all.csv')` contains the following variables:
