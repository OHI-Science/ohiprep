...
iucn_spp_list <- read_csv(file.path(dir_goal, 'int', 'spp_marine_all.csv')) %>%
...
am_spp_list <- read_csv(file.path(dir_anx, '_raw_data', 'aquamaps/d2015/csv', 
                                  'speciesoccursum.csv')) %>%
...
am_not_matched_valid <- am_not_matched %>%
  ...
  mutate(iucn_sid = iucn_id) %>%
  rename(am_iucn_id = iucn_id)
...
Check these; they all have IUCN species IDs but do not seem to be counted as marine?

`r DT::datatable(am_not_matched)`
