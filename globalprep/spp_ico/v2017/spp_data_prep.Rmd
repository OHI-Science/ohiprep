...
* __Reference__: 
    ...
    * BirdLife International and NatureServe (2016) Bird species distribution maps of the world. BirdLife International, Cambridge, UK and NatureServe, Arlington, USA.
        ...
        * __Downloaded__: 23 December 2015.
...
This process is done in `4_ingest_past_assessments.Rmd`, and results are saved to saved to `~/github/ohiprep/globalprep/spp_ico/v2017/int/spp_cat_trend_timeseries.csv`.

* `iucn_sid`, `risk_trend`, `year`, `category`, `cat_score`, `assess_year`
...
Attach the general species list to the time series of category/trend scores.  Manually fill in those few AquaMaps species without an IUCN ID match.
...
am_cells_spp_sum_list <- mclapply(years, mc.cores = length(years), FUN = function(yr) {
...
write_csv(am_cells_spp_sum, file.path(dir_goal_anx, 'summary/am_cells_spp_summary.csv'))

iucn_cells_spp_sum_list <- mclapply(years, mc.cores = length(years), FUN = function(yr) {
...
write_csv(iucn_cells_spp_sum, file.path(dir_goal_anx, 'summary/iucn_cells_spp_summary.csv'))

```

``` {r generate_means_per_cell}

iucn_cells_spp_sum <- read_csv(file.path(dir_goal_anx, 'summary/iucn_cells_spp_summary.csv'))
am_cells_spp_sum   <- read_csv(file.path(dir_goal_anx, 'summary/am_cells_spp_summary.csv'))
...
sum_by_loiczid <- read_csv(list.files(file.path(dir_goal_anx, 'summary'), 
                                   pattern = 'cell_spp_sum', 
                                   full.names = TRUE))
