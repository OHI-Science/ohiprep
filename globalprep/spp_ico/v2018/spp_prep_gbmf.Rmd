...
if(!file.exists(tot_sum_gbmf_file)) {
  ...
  am_cell_area <- read_tsv(file.path(dir_data_am, 'csv/hcaf_v6.csv')) %>%
      select(loiczid = LOICZID, cell_area = CellArea, ocean_area = OceanArea)
...
ggplot(spp_list_all, aes(x = cat_score)) +
  theme_bw() +
  theme(panel.border = element_blank()) +
  geom_bar() +
  facet_wrap( ~ rgn_name, scales = 'free_y') +
  labs(x = 'Extinction risk (0 = Least Concern, 1 = Extinct)',
       y = 'Number of species in rgn')
ggsave(file.path(dir_goal, 'gbmf_maps/risk_spp_barchart.png'))

```

-----

## Map mean risk scores, spp status, and n_spp for each region

``` {r calc_status_for_gaviota}

am_cell_area <- read_tsv(file.path(dir_data_am, 'csv/hcaf_v6.csv')) %>%
    select(loiczid = LOICZID, cell_area = CellArea, ocean_area = OceanArea)

gaviota_status <- read_csv(file.path(dir_goal, 'summary/spp_summary_gbmf.csv')) %>%
  left_join(am_cell_area, by = 'loiczid') %>%
  filter(rgn_id == 1000) %>%
  group_by(rgn_name) %>%
  summarize(rgn_mean_risk = sum(mean_risk * ocean_area) / sum(ocean_area),
            rgn_status = 100 * (.75 - rgn_mean_risk) / .75)
...
cells_sum <- read_csv(file.path(dir_goal, 'summary/spp_summary_gbmf.csv')) %>%
  mutate(status = 100 * (.75 - mean_risk) / .75)
...
plot_rast_map <- function(shp, cells_df, land_shp, column) {
  ...
  tmp_plot <- ggplot() +
    ...
    geom_sf(data = shp, aes(), fill = NA, color = 'grey30', size = .5) +
    geom_sf(data = land_shp, aes(), fill = 'grey40', color = 'grey30', size = .5) +
    xlim(c(ext[1:2])) +
    ylim(c(ext[3:4])) +
...
for(rgn in rgns) {
  ...
  map_risk <- plot_rast_map(shp, df, land_shp, column = 'mean_risk') +
    ...
    labs(title = sprintf('%s, mean extinction risk', rgn_nm),
         fill = 'Risk: 0 = LC, 1 = EX')
  map_status <- plot_rast_map(shp, df, land_shp, column = 'status') +
    scale_fill_distiller(palette = 'RdYlGn', direction = 1, 
                         limits = c(84, 100)) +
    labs(title = sprintf('%s, OHI Species status', rgn_nm),
         fill = 'Status: 0 - 100')
  ...
  ggsave(file.path(dir_goal, 'gbmf_maps', sprintf('%s_status.png', tolower(rgn_nm))), 
                   dpi = 300, plot = map_status)
}
...
risk_rast  <- subs(loiczid_rast, cells_sum, by = 'loiczid', which = 'mean_risk',
                   ...
                   overwrite = TRUE)
status_rast <- subs(loiczid_rast, cells_sum, by = 'loiczid', which = 'status',
                   filename = file.path(dir_goal, 'gbmf_maps/raster_status.tif'),
                   overwrite = TRUE)
