...
am_dir_2014 <- file.path(dir_neptune_data, 'git-annex/globalprep/_raw_data/aquamaps/v2014/raw')
am_dir_2015 <- file.path(dir_neptune_data, 'git-annex/globalprep/_raw_data/aquamaps/v2015/aquamaps_2015_full_dataset_ohi')
...
am_file_hcaf <- file.path(am_dir_2015, 'hcaf_ohi.sql')
# am_file_hcaf <- file.path(am_dir_2014, 'hcaf.sql')

system.time({
  sql_df <- data.frame('raw' = scan(file = am_file_hcaf,      
                                    what = 'character', 
                                    nmax = -1, sep = '\n'))
...
# am_file_sp_occur <- file.path(am_dir_2014,  'ohi_speciesoccursum.sql')
am_file_sp_occur <- file.path(am_dir_2015,  'speciesoccursum_ohi.sql')

system.time({
  sql_df <- data.frame('raw' = scan(file = am_file_sp_occur,  
                                    what = 'character', 
                                    nmax = -1, sep = '\n'))
...
# am_file_sp_native <- file.path(am_dir_2014, 'ohi_hcaf_species_native.sql')
am_file_sp_native <- file.path(am_dir_2015, 'hcaf_species_native_ohi.sql')

system.time({
  nlines_query <- system2(command = 'wc', args = sprintf('-l %s', am_file_sp_native), stdout = TRUE)
...
csq_to_loiczid <- fread(file.path(am_dir_2015, '../csv/hcaf_truncated.csv')) %>% 
  select(csquarecode, loiczid) %>%
  data.table(key = 'csquarecode')
n_records <- 0
n_window  <- 15e6 ### try 15M line windows for better speed?
sp_cells_file <- file.path(am_dir_2015, '../csv/hcaf_sp_native_trunc.csv')
...
for (i in 1:ceiling(nlines/n_window)) { # i = 1
  ...
  system.time({
    ...
    sql_df <- data.table('raw' = scan(file = am_file_sp_native,
                                      what = 'character',
                                      skip = read_skip,
  ...
  message(sprintf('Writing %s records (total = %s records) to:\n  %s', 
          nrow(sp_cells_df), n_records, sp_cells_file))
  write.table(sp_cells_df, sp_cells_file, sep = ',', qmethod = 'double',
...
# x <- fread(sp_cells_file) ### reads 1.686 GB in 26 seconds
