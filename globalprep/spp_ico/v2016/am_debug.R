...
am_cs15b <- process_am_summary_per_cell(spp_all, spp_cells = am_cs15, prob_filter = .40, fn_tag = 'amtesting15_40')
am_cs15b0 <- process_am_summary_per_cell(spp_all, spp_cells = am_cs15, prob_filter = 0,  fn_tag = 'amtesting15_0')

am_cs16b <- process_am_summary_per_cell(spp_all, spp_cells = am_cs16, prob_filter = 0, fn_tag = 'amtesting16')

am_2015 <- process_means_per_rgn(am_cs15b %>%
                                   rename(weighted_mean_cat   = mean_cat_score,
                                          weighted_mean_trend = mean_pop_trend_score), 
                                 rgn_cells, rgn_note = 'amtesting15_40') 
am_2015_0 <- process_means_per_rgn(am_cs15b0 %>%
                                   rename(weighted_mean_cat   = mean_cat_score,
                                          weighted_mean_trend = mean_pop_trend_score), 
                                 rgn_cells, rgn_note = 'amtesting15_0') 
am_2016 <- process_means_per_rgn(am_cs16b %>%
                                   rename(weighted_mean_cat   = mean_cat_score,
                                          weighted_mean_trend = mean_pop_trend_score),
                                 rgn_cells, rgn_note = 'amtesting16') 
...
both_cs15 <- process_means_per_cell(am_cs15b, iucn_cs15b, fn_tag = 'both15')
both_cs15_0 <- process_means_per_cell(am_cs15b0, iucn_cs15b, fn_tag = 'both15_0')
both_cs16 <- process_means_per_cell(am_cs16b, iucn_cs16b, fn_tag = 'both16')

both2015 <- process_means_per_rgn(both_cs15, rgn_cells, rgn_note = 'bothtesting15')
both2015_0 <- process_means_per_rgn(both_cs15_0, rgn_cells, rgn_note = 'bothtesting15_0')
both2016 <- process_means_per_rgn(both_cs16, rgn_cells, rgn_note = 'bothtesting16')
...
iucn <- ggplot(iucn_1516) +
  geom_point(aes(x = st_2015, y = st_2016),
             col = 'blue', alpha = .8) +
  geom_abline(slope = 1, intercept = 0, col = 'red', alpha = .5) +
  scale_x_continuous(limits = c(.5, 1)) +  
  scale_y_continuous(limits = c(.5, 1)) +
  labs(title = 'IUCN 2015 vs 2016',
       x = 'SPP status v2015 (2014 data)',
       y = 'SPP status v2016 (2015 data)')

both <- ggplot(both_1516) +
  geom_point(aes(x = st_2015_old, y = st_2016),
             col = 'grey30', alpha = .5) +
  geom_point(aes(x = st_2015, y = st_2016),
             col = 'blue', alpha = .8) +
...
# x <- x + 
#   geom_point(data = status_df, 
#              aes(x = st_2015, y = st_2016),
#              col = 'black', alpha = .3)
