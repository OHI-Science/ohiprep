...
iucn_cs15b <- process_iucn_summary_per_cell(spp_all, spp_cells = iucn_cs15, fn_tag = 'testing15') %>%
  rename(weighted_mean_cat   = mean_cat_score,
         weighted_mean_trend = mean_pop_trend_score)

iucn_cs16b <- process_iucn_summary_per_cell(spp_all, spp_cells = iucn_cs16, fn_tag = 'testing16')

iucn2015 <- process_means_per_rgn(iucn_cs15b %>%
                                    rename(weighted_mean_cat   = mean_cat_score,
                                           weighted_mean_trend = mean_pop_trend_score),
                                  rgn_cells_hi, rgn_note = 'testing15') 
iucn2016 <- process_means_per_rgn(iucn_cs16b %>%
                                    rename(weighted_mean_cat   = mean_cat_score,
                                           weighted_mean_trend = mean_pop_trend_score),
                                  rgn_cells_hi, rgn_note = 'testing16') 
