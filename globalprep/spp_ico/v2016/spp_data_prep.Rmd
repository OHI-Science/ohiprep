...
mc_get_from_api <- function(url, param_vec, api_key, delay = 1.0) {
...
if(!file.exists(spp_list_from_api_file)) {
  ...
  git_prov(spp_list_from_api_file, filetype = 'output')
}
...
spp_habs_from_api_file <- file.path(dir_goal, 'int/spp_habs_from_api.csv')

if(!file.exists(spp_habs_from_api_file)) {
  message('Using API to determine species habitats from full species list')
  
  spp_ids_all <- read_csv(spp_list_from_api_file) %>%
    .$iucn_sid
  ...
  ### 500 spp takes 184 seconds; 87851 species should take about 32000 
  ###   seconds (9 hours).  Each chunk will save to tmp for later combining.
  
  chunk_size <- 20
  n_chunks <- 5 # ceiling(length(spp_ids_all)/chunk_size)
  
  for(j in 1:n_chunks) { ### j <- 7
    spp_index <- c( ((j - 1) * chunk_size + 1) : min((j * chunk_size), length(spp_ids_all)) )
    spp_chunk <- spp_ids_all[spp_index]
    ...
    if(!file.exists(chunk_file)) {
      ### species for this chunk:
      message('Getting habitat info for species ', min(spp_index), ' to ', max(spp_index))
      
      spp_habs_chunk <- mc_get_from_api(spp_habs_url, spp_chunk, api_key, delay = 1)
      
      spp_habs_chunk <- spp_habs_chunk %>%
        rename(iucn_sid = id, hab_code = code)
      
      write_csv(spp_habs_chunk, 
                file.path(dir_goal, 'tmp', 
                          sprintf('spp_habs_chunk_%s_%s.csv', 
                                  min(spp_index), max(spp_index))
                          )
                )
