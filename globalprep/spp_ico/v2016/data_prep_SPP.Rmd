...
`r knitr::kable(peek_csv(file.path(dir_data_am, 'csv/hcaf_truncated.csv')), caption = 'Half-degree cell info from hcaf_truncated.csv (first few rows)')`

`r knitr::kable(peek_csv(file.path(dir_data_am, 'csv/speciesoccursum.csv')), caption = 'Species info from speciesoccursum.csv (first few rows)')`

`r knitr::kable(peek_csv(file.path(dir_data_am, 'csv/hcaf_sp_native_trunc.csv')), caption = 'Species-to-cell lookup from hcaf_sp_native_trunc.csv (first few rows)')`
...
``` {r Ingest IUCN species list, eval = TRUE}

source(file.path(goal, 'R/ingest_iucn.R'))

```

`r knitr::kable(peek_csv(file.path(dir_anx, scenario, 'int/spp_iucn_marine.csv')), caption = 'IUCN marine species list and info from spp_iucn_marine.csv (first few rows)')`
...
am_cells_spp_sum <- process_am_summary_per_cell(spp_all, fn_tag = '', prob_filter = 0, reload = TRUE) %>%
  read_csv()
...
iucn_cells_spp_sum <- process_iucn_summary_per_cell(spp_all, fn_tag = '', reload = TRUE) %>%
  read_csv()
### loiczid | mean_cat_score | mean_trend_score | n_cat_species | n_trend_species
...
``` {r child = file.path('~/github/ohibc', 'src/templates/ohibc_prov_ftr.Rmd')}
```
