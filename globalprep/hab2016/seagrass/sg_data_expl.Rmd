...
waycott_df <- read.csv(data_waycott_file, stringsAsFactors = FALSE)
...
waycott_df_mdl2 <- waycott_df %>%
  ...
  mutate(exp_mdl_r = round(log(last(area_ha) / first(area_ha)) / (last(year) - first(year)) * 100, 3))

waycott_df <- waycott_df %>%
  left_join(waycott_df_mdl %>% select(-mdl),
            by = 'site_name') %>%
  left_join(waycott_df_mdl2 %>% 
              select(site_name, year, exp_mdl_r),
            by = c('site_name', 'year'))

DT::datatable(waycott_df %>% select(-site_id, -yr_start, -yr_end, -pub),
