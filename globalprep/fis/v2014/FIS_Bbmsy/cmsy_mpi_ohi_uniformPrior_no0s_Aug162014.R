...
cdat<-read.csv("OHICatchHistoryCMSY_07_21_2014.csv", stringsAsFactors=FALSE)
##head(cdat)
...
runCMSY<-function(){
  ...
  finalbio    <- c(0.01,0.7) ## use for batch processing   ****THIS WAS CHANGED FROM RAINER's VERSION--Oct 12, 2012
...
cmsy.ohi.unif.no0.df<-do.call(rbind,tmp0)

rownames(cmsy.ohi.unif.no0.df)<-NULL

file.string<-paste("./cmsy_ohi_results_table.RData", sep="")
save(list = c("cmsy.ohi.unif.no0.df"), file = file.string)
