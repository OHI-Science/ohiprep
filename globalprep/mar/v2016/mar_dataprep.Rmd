...
#Summary
This analysis converts FAO mariculture data into the OHI 2016 global mariculture status score.
...
#Data Source 
**Reference**:      http://www.fao.org/fishery/statistics/software/fishstatj/en#downlApp
...
source('globalprep/mar/v2016/mar_fxs.R') # functions specific to mariculture
source('globalprep/np/v2016/R/np_fxn.R')  
source('src/R/fao_fxn.R') # function for reading FAO files
...
mar <- read.csv('C:/Users/Melanie/Desktop/FAO_mariculture/d2016/GlobalAquacultureProduction_Quantity_1950_2014.csv', check.names=F, stringsAsFactors=F) ; head(mar) 
...
mar_sp <- read.csv('globalprep/mar/v2016/raw/species_list.csv', stringsAsFactors=FALSE) %>%
...
# remove extra fields, and restore table name
mar = mar %>% 
...
mar <- mar %>%
  mutate(country = ifelse(country=="Réunion", "Reunion", country)) %>%  # this one is hard to get right
...
write.csv(maric, 'globalprep/mar/v2016/output/MAR_FP_data.csv', row.names=FALSE)
...
sus = read.csv('globalprep/mar/v2016/raw/Truj_label_sust.csv', stringsAsFactors = FALSE, na.strings = NA) # upload file with trujillo scores by label
...
write.csv(mar_harvest_tonnes, 'globalprep/mar/v2016/output/mar_harvest_tonnes.csv', row.names=F)

mar_harvest_tonnes_gf = mar_sus_final %>%
  select(rgn_id, taxa_code, year, tonnes=gapfill_fao)
write.csv(mar_harvest_tonnes, 'globalprep/mar/v2016/output/mar_harvest_tonnes_gf.csv', row.names=F)
...
write.csv(mar_sustainability_score, 'globalprep/mar/v2016/output/mar_sustainability.csv', row.names=F)

mar_sustainability_score_gf = mar_sus_final %>% 
  select(rgn_id, taxa_code, sust_coeff = gapfill_sus) %>% 
  unique()
write.csv(mar_sustainability_score, 'globalprep/mar/v2016/output/mar_sustainability_gf.csv', row.names=F)
...
genEscapes_gf <- tonnes_esc %>%
  ...
  data.frame()
write.csv(genEscapes_gf, 'globalprep/mar/v2016/data/GenEsc_v2016_gf.csv', row.names=FALSE)
...
data_year <- year-2
    ...
    select(rgn_id, genEscapes)
write.csv(data, sprintf('globalprep/mar/v2016/output/GenEsc_v%s.csv', year), row.names=FALSE)  
}
