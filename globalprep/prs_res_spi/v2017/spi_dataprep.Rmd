...
spi_calc <- spi_calc %>%
  mutate(Country = ifelse(Country=="C\xf4te d'Ivoire", "Cote d'Ivoire", Country),
         ...
         Country = ifelse(Country=="Curaçao", "Curacao", Country),
...
mod2 <- lm(score ~ r2 + wgi_score, data = rgns_gf_un)
...
abline(0,1)
plot(0:100, predict(mod7, newdata = data.frame(r1=1, wgi_score=seq(0, 1, by=0.01))), type="l")
points(rgns_gf_un$wgi_score*100, rgns_gf_un$score)
plot(0:100, predict(mod6, newdata = data.frame(r1=1, wgi_score=seq(0, 1, by=0.01))), type="l")
points(rgns_gf_un$wgi_score*100, rgns_gf_un$score)
...
mod_gf <- lm(score ~ r1 + poly(wgi_score, 3), data = rgns_gf_un, na.action = na.exclude)
rgns_gf_un$score_pred <- predict(mod_gf, newdata = rgns_gf_un[, c("r1", "wgi_score")])

rgns_gf <- rgns_gf_un %>%
  mutate(method = ifelse(is.na(score) & !is.na(score_pred), "UN georgn & WGI", method)) %>%
  mutate(gapfill = ifelse(is.na(score) & !is.na(score_pred), "1", gapfill)) %>%
