...
read_spp_iucn <- function(reload = FALSE) {
  ...
  if(!file.exists(spp_iucn_processed_file) | reload) {
    ...
    spp_iucn <- read_csv(iucn_marine_raw_file) %>%
      dplyr::select(sciname, iucn_sid, iucn_cat = category, pop_trend, parent_sid, subpop_sid) 
...
create_spp_master_lookup <- function(source_pref = 'iucn', fn_tag = '', reload = FALSE) {
  ...
  if(!file.exists(spp_all_file) | reload) {
    ...
    spp_all1 <- spp_all1 %>%
      ...
      left_join(pop_trend, by = 'popn_trend') 
...
ditch_dupes <- function(spp_all) {
  ...
  spp_all1 <- spp_all1 %>%
    ...
    unique()
  spp_all1 %>% filter(sciname %in% spp_all1$sciname[dupes]) %>% select(spp_group) %>% unique()
