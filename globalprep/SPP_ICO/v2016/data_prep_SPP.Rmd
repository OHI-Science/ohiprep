...
for(fn in intsx_files) { # fn <- intsx_files[1]
  ...
  message(fn, ': ', paste(names(x), sep = ', '))
  if('id_no' in names(x)) x <- x %>% rename(iucn_sid = id_no)
  if(!'subpop' in names(x)) x <- x %>% mutate(subpop = NA)
  message('Writing ', fn, ': ', paste(names(x), sep = ', '))
...
extract_loiczid_per_spp(maps_list_dd,
                        fn_tag = paste('dd_', scenario, sep = ''),
                        reload = TRUE)
