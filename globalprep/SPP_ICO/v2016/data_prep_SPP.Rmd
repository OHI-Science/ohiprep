...
### set up maps_list for DD species...
### go into all intersection .csvs, and pull all iucn_sid, and see which
### spp still aren't accounted for?
### also fix col names to new: iucn_sid instead of id_no? add subpop col?
cache_dir <- file.path(dir_anx, 'iucn_intersections')
intsx_files <- list.files(cache_dir)
y <- c()
for(fn in intsx_files) { # fn <- intsx_files[1]
  write_flag <- FALSE
  x <- read_csv(file.path(cache_dir, fn))
  message(fn, ': ', paste(names(x), collapse = ', '))
  if('id_no' %in% names(x)) {
    x <- x %>% rename(iucn_sid = id_no)
    write_flag <- TRUE
  }
  if(!'subpop' %in% names(x)) {
    x <- x %>% mutate(subpop = NA)
    write_flag <- TRUE
  }
  if(write_flag) {
    message('Writing ', fn, ': ', paste(names(x), collapse = ', '))
    write_csv(x, file.path(cache_dir, fn))
  }
  x_sid <- x$iucn_sid %>% unique()
  y <- c(y, x_sid)
}

z <- spp_all %>%
  filter(!iucn_sid %in% y) %>%
  filter(str_detect(spatial_source, 'iucn'))

zz <- spp_all %>% filter(popn_category != 'DD') %>%
  filter(str_detect(spp_group, 'SEABREAM'))

maps_list_dd <- spp_all %>%
  filter(str_detect(spatial_source, 'iucn') & !str_detect(spatial_source, 'bli')) %>%
  filter(popn_category == 'DD') %>%
  select(sciname, iucn_sid, spp_group) %>%
  unique()

extract_loiczid_per_spp(zz,
                        fn_tag = scenario,
                        reload = TRUE)
