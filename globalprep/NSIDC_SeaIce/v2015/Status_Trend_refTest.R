...
#   z.p.T <- z.p.T[!is.na(z.p.T$Status_2011_pctdevR), ]  
  ...
  write.csv(z.h.T, file.path(github, sprintf('RefYearTest/%s_IceEdgeHabitat_ref%sto%s.csv', p, min(ref.years), max(ref.years))), row.names=FALSE)
  write.csv(z.p.T, file.path(github, sprintf('RefYearTest/%s_IceShoreProtection_ref%sto%s.csv', p, min(ref.years), max(ref.years))), row.names=FALSE) 
